...
def goTo(lat,lng):
		...
		client = actionlib.SimpleActionClient('turtlebot/move_base', MoveBaseAction)   
...
rospy.Subscriber("turtlebot/lat", String, lat_callback)
def lng_callback(sensor_value_json):
	sensor_value = json.loads(sensor_value_json.data)
	for topic in my_sensor_data["topics"]:
		if topic.get("topic_name") == "lng":
			topic["topic_value"] = sensor_value
rospy.Subscriber("turtlebot/lng", String, lng_callback)
my_actions_table = {
        "actions": [
        {
          "action_name" : "goTo",
